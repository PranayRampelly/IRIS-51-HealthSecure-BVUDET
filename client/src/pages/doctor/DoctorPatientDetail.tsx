...
import { ArrowLeft, Shield, FileText, Calendar, Phone, Mail, Activity } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview" className="space-y-6">
                ...
                <CardContent>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-health-charcoal">DOB: {patient?.dateOfBirth ? new Date(patient.dateOfBirth).toLocaleDateString() : 'N/A'}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Phone className="w-4 h-4 text-health-teal" />
                      <span className="text-sm text-health-charcoal">{patient?.phone || 'N/A'}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Mail className="w-4 h-4 text-health-teal" />
                      <span className="text-sm text-health-charcoal">{patient?.email || 'N/A'}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {patient?.primaryDiagnosis && <Badge variant="outline">{patient.primaryDiagnosis}</Badge>}
                        {patient?.secondaryDiagnosis?.map((d: string, i: number) => (
                          <Badge key={i} variant="outline">{d}</Badge>
                        ))}
                        {!patient?.primaryDiagnosis && !patient?.secondaryDiagnosis?.length && <span className="text-sm text-gray-500">No conditions recorded</span>}
                      </div>
                    </div>
                    <div>
                      ...
                      <ul className="text-sm text-health-charcoal space-y-1">
                        {patient?.currentMedications?.length > 0 ? (
                          patient.currentMedications.map((med: any, i: number) => (
                            <li key={i}>• {med.medication} {med.dosage}</li>
                          ))
                        ) : (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-health-charcoal">
                        {patient?.updatedAt ? new Date(patient.updatedAt).toLocaleDateString() : 'No visits recorded'}
                      </p>
                    </div>
                  </div>
                </CardContent>
        ...
        <TabsContent value="analytics">
          {patient ? (
            <ChronicDiseaseTracker patientId={patient.patientId || patient._id} />
          ) : (
