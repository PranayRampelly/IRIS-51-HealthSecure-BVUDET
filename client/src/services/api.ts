...
class ApiService {
  baseURL: string;
  requestQueue: Map<string, NodeJS.Timeout>;
  retryDelay: number;
  ...
  async get(url: string, config: RequestConfig = {}): Promise<any> {
    try {
      ...
      if (error.message?.includes('Failed to fetch') || error.message?.includes('ERR_CONNECTION_REFUSED')) {
        throw new Error(`Connection refused. Please ensure the server is running on ${this.baseURL.replace('/api', '')}`);
      }
      throw error;
    }
  }

  async post(url: string, data: any = null, config: RequestConfig = {}): Promise<any> {
    const response = await fetch(`${this.baseURL}${url}`, {
      method: 'POST',
      headers: {
        ...this.getAuthHeaders(),
  ...
  async debouncedRequest(key: string, requestFn: () => Promise<any>, delay: number = 1000): Promise<any> {
  ...
  async login(credentials) {
    try {
      ...
      // Store token if login was successful and token is provided
      if (data.token) {
  ...
  async login2FA(credentials) {
    try {
      ...
      // Store token if 2FA login was successful
      if (data.token) {
        this.setToken(data.token);
      }
  ...
  async getHealthRecords(params: Record<string, any> = {}) {
  ...
  async createProof(data: any) {
    // Check if we're passing just a recordId string or a full proof object
    const endpoint = typeof data === 'string' ? `${this.baseURL}/proofs/record` : `${this.baseURL}/proofs`;
    const body = typeof data === 'string' ? JSON.stringify({ recordId: data }) : JSON.stringify(data);
